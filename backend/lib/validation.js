 ...
 * Mirrors frontend validation.ts for consistency
 */
 ...
 *   - Case insensitive
 */
export function validateLinkedInUrl(url, required = false) {
 ...
 */
export function validateResume(resumeText, required = false) {
 ...
 */
export function validateSelfDescription(description, required = false) {
 ...
 */
export function validateDesiredRoles(roles) {
...
/**
 * Validate core required fields for proceeding (progressive validation)
 * Full Name, Email, At least 1 Role, Location, Experience
 */
export function validateCoreFields(data) {
  const errors = {};
...
export function validateProfileInput(data) {
  ...
  // Desired roles validation (1-3 roles)
  const rolesResult = validateDesiredRoles(data.desiredRoles);
  if (!rolesResult.valid) errors.desiredRoles = rolesResult.error;
...
export function formatValidationErrors(errors) {
  const messages = Object.entries(errors).map(([field, error]) => `${field}: ${error}`);
  return {
